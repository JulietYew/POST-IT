const avatarStyles = [
  ...
  const getRandomAvatarStyle = () => {
    // Your code here
    //Returns a random integer between 0 and  length of avatarStyles
    const randomIndex = Math.floor(Math.random() * avatarStyles.length); 
    // returns a new array containing a single element at randomIndex and picks the first element from the array.
